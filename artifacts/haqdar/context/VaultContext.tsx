...
export function VaultProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (deviceId && isUnlocked) syncFromCloud();
  ...
  const syncFromCloud = useCallback(async () => {
    ...
    try {
      ...
      for (const inc of localOnly) {
        try { await pushIncidentToCloud(inc, deviceId); } catch { /* non-fatal */ }
      }

      const cloudOnly = cloudIncidents.filter((i) => !localIds.has(i.id));
      for (const inc of cloudOnly) {
        try { await saveIncident(inc); } catch { /* non-fatal */ }
      }
  ...
  const unlock = useCallback(async (pin: string): Promise<boolean> => {
    try {
      ...
      if (entered === stored) { setIsUnlocked(true); return true; }
      return false;
    } catch { return false; }
  }, []);
  ...
  const addIncident = useCallback(
    async (data: Omit<Incident, "id" | "hash" | "timestamp">): Promise<Incident> => {
      ...
      const hashInput = `${timestamp}|${data.narrative}|${data.media.map((m) => m.filename).join(",")}`;
      ...
      if (deviceId) {
        pushIncidentToCloud(incident, deviceId).catch((e) =>
          console.warn("[VaultContext] cloud push failed:", e)
        );
      }
  ...
  const removeIncident = useCallback(
    async (id: string) => {
      ...
      if (deviceId) { deleteIncidentFromCloud(id).catch(() => {}); }
      await refresh();
    },
    [refresh, deviceId]
  );
  ...
  const wipe = useCallback(async () => {
    if (deviceId) { deleteAllIncidentsFromCloud(deviceId).catch(() => {}); }
    await wipeAllData();
    setIsUnlocked(false);
    setIncidents([]);
    setPinIsSet(false);
  ...
  return (
    <VaultContext.Provider
      value={{
        isUnlocked, pinIsSet, incidents, loading, syncing, cloudConnected,
        unlock, lock, setupPin, addIncident, removeIncident, wipe, refresh, syncFromCloud,
      }}
    >
      {children}
    </VaultContext.Provider>
